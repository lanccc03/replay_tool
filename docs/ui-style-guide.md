...
本文定义后续 PySide6 UI 的视觉风格、色彩、布局和组件使用规则。当前项目尚未实现 Qt UI；本指南用于约束未来 `replay_ui_qt` 工作台设计，不表示相关界面已经完成。
...
- 左侧导航用于一级模块：Trace Library、Scenarios、Replay Monitor、Devices、Settings。
