...
def On_pre_CANFD_EVENT(OBJ, ACANFD):
    ...
    if ACANFD.contents.FIdentifier == 0x3FC:
        if key == 0:
            tsdb_set_signal_value_canfd(ACANFD.contents, "INV_Data".encode("utf8"), "MotorSpeed".encode("utf8"),
                                        c_double(10.0))
        elif key == 1:
            tsdb_set_signal_value_canfd(ACANFD.contents, "INV_Data".encode("utf8"), "MotorSpeed".encode("utf8"),
                                        c_double(10.0))
...
def On_CANFD_EVENT(OBJ, ACANFD):
    if ACANFD.contents.FFDProperties == 1 and ACANFD.contents.FIdentifier == 0X64:

        tsdb_get_signal_value_canfd(ACANFD.contents, "INV_Data".encode("utf8"), "MotorSpeed".encode("utf8"),
                                    signalvalue)
...
def connect():
    tsdb_load_can_db((_curr_path+"/CAN_FD_Demo.dbc").encode('utf8'), "0,1".encode('utf8'), dbchandle)
    # 设置CAN通道
