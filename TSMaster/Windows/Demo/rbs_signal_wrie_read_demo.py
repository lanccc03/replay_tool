...
def connect():
    ret = tsdb_load_can_db((_curr_path+"/CAN_FD_Demo.dbc").encode("utf8"), b"0,1", dbchandle)
    print(ret,dbchandle)
    # 设置can通道数
    ...
    if ret == 0:
        ...
        # 激活network
        tscom_can_rbs_activate_network_by_name(0, True, b"CAN_FD_Demo", False)
        # 激活节点
        tscom_can_rbs_activate_node_by_name(0, True, b"CAN_FD_Demo", b"INV", False)
        # 发送指定报文
        ret = tscom_can_rbs_activate_message_by_name(0, True, b"CAN_FD_Demo", b"INV",
                                                    b"INV_Data")
    else:
        error_code = c_char_p()
...
if __name__ == "__main__":
    ...
    while True:
        key = input("请输入key值")
        if key == "1":
            tscom_can_rbs_set_signal_value_by_element(0, "CAN_FD_Demo".encode("utf8"), "INV".encode("utf8"),
                                                      "INV_Data".encode("utf8"), "MotorSpeed".encode("utf8"),
                                                      c_double(1.0))

            time.sleep(0.1)
            tscom_can_rbs_get_signal_value_by_element(0, "CAN_FD_Demo".encode("utf8"), "INV".encode("utf8"),
                                                      "INV_Data".encode("utf8"), "MotorSpeed".encode("utf8"),
                                                      signalvalue)
            print(signalvalue)
            ...
            tscom_can_rbs_activate_network_by_name(0, False, "CAN_FD_Demo".encode("utf8"), True)
            break
        else:
            tscom_can_rbs_set_signal_value_by_element(0, "CAN_FD_Demo".encode("utf8"), "INV".encode("utf8"),
                                                      "INV_Data".encode("utf8"), "MotorSpeed".encode("utf8"),
