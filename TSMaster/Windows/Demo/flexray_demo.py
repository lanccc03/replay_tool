...
flexraydb_AId = TSMasterAPI.c_int32(0)
TSMasterAPI.tsdb_load_flexray_db(b"D:\\IDE\\TSMasterAPI\\TSMasterApi\\databases\\FlexRay_Demo.xml",b'0',flexraydb_AId)
def e2e_crc8(data):
    polynomial = 0x1d
    initValue = 00^0X00
...
TSMasterAPI.tscom_flexray_get_signal_definition(b'0/Edrive/LCU/LightingInfo/StopLamp',TSignal_)
# 该函数为回调事件 可以查看报文实时记录
def Flexray_RX(obj,AFlexray):
    ...
    '''
    #(16,0,2)
    if(AFlexray.contents.FSlotId == 51 and AFlexray.contents.FCycleNumber%2==0):
...
ret = TSMasterAPI.tscom_flexray_rbs_activate_cluster_by_name(1,True,b"Edrive",False)
ret = TSMasterAPI.tscom_flexray_rbs_activate_ecu_by_name(1,True,b"Edrive",b"LCU",True)
...
while 1:
    ...
    if key == 'q':
        ...
        if AValue>14:
            AValue = 0.0
        ret = TSMasterAPI.tscom_flexray_rbs_set_signal_value_by_address(b'0/Edrive/EBC/LightingInfo/StopLamp',ctypes.c_double(AValue))
        print('AValue = ',AValue)
        signal_value =  TSMasterAPI.tscom_flexray_rbs_get_signal_value_by_address(b'0/Edrive/EBC/LightingInfo/StopLamp')
